...
export const revertChangeset = (obj: any, changeset: Changeset) => {
  if (changeset) {
    changeset.reverse().forEach((change: IChange): any => (!change.changes ? revertLeafChange(obj, change) : revertBranchChange(obj[change.key], change)))
  }
  return obj
}
...
export const flattenChangeset = (obj: Changeset | IChange, path = '$', embeddedKey?: string | FunctionKey): IFlatChange[] => {
  if (Array.isArray(obj)) {
